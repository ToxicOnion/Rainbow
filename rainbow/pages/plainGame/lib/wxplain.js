...
function flyGame(opts) {
  ...
  async function drawBg() {
    ...
    //     bg_img_width = res.screenHeight
    //     bg_img_height = res.screenWidth

    //   },
    // })
    // if (!config.downPhoto) {
    //   config.downPhoto = true
    //   let result = await getimageinfo(flyimages)
    //   console.log(result)
    //   for (var i = 0; i < result.length; i++) {
    //     flyimages[result[i].key].src = result[i].temUrl
    //   }
    // }
    cxt.drawImage(flyimages["bg"].src, 0, 0, bg_img_width, bg_img_height)
  }
  //构造logo
  function drawLogo() {
